...
class PickerAdapter : RecyclerView.Adapter<PickerItemViewHolder>() {
    ...
    override fun onBindViewHolder(
        ...
        when (selectedItem) {
            position -> {
                holder.tvItem?.setTextColor(ContextCompat.getColor(ctx!!, R.color.tab_select_text))
                selectedItem = -1
            }
            else -> holder.tvItem?.setTextColor(ContextCompat.getColor(ctx!!, R.color.tab_select_text))
        }
    }
